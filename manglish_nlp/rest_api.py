...
app = FastAPI(
    ...
    version=getattr(manglish_nlp, '__version__', '3.0.0'),
    docs_url="/docs",
    redoc_url="/redoc",
)
...
def process_with_timing(func, *args, **kwargs):
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy",
        version=getattr(manglish_nlp, '__version__', 'unknown'),
        modules_loaded=len([t for t in manglish_nlp.available_tasks()]) if hasattr(manglish_nlp, 'available_tasks') else 51,
    )
...
async def analyze_text(req: TextRequest):
    ...
    result = {
        "normalized": manglish_nlp.normalize(req.text),
        "sentiment": manglish_nlp.sentiment(req.text),
        "language": manglish_nlp.detect_language(req.text),
        "pos_tags": manglish_nlp.pos_tag(req.text),
        "entities": manglish_nlp.ner_tag(req.text),
        "emotion": manglish_nlp.detect_emotion(req.text),
        "keywords": manglish_nlp.extract_keywords(req.text),
    }
...
async def sentiment_analysis(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.sentiment, req.text)
...
async def normalize_text(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.normalize, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def translate_text(req: TranslateRequest):
    ...
    if target == 'en':
        result, ms = process_with_timing(manglish_nlp.to_english, req.text)
    elif target in ('bm', 'ms', 'malay'):
        result, ms = process_with_timing(manglish_nlp.to_malay, req.text)
    elif target == 'formal':
        result, ms = process_with_timing(manglish_nlp.to_formal, req.text)
    else:
        result, ms = process_with_timing(manglish_nlp.translate, req.text)
...
async def named_entity_recognition(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.ner_tag, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))


@app.post("/pos", response_model=NLPResponse, tags=["NLP"])
async def pos_tagging(req: TextRequest):
    """Part-of-Speech tagging."""
    result, ms = process_with_timing(manglish_nlp.pos_tag, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def summarize_text(req: TextRequest):
    """Summarize text."""
    result, ms = process_with_timing(manglish_nlp.summarize, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def detect_emotion(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.detect_emotion, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def extract_keywords(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.extract_keywords, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def detect_language(req: TextRequest):
    ...
    result, ms = process_with_timing(manglish_nlp.detect_language, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))


@app.post("/formalize", response_model=NLPResponse, tags=["NLP"])
async def formalize_text(req: TextRequest):
    """Convert informal text to formal BM."""
    result, ms = process_with_timing(manglish_nlp.formalize, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))


@app.post("/dialect", response_model=NLPResponse, tags=["NLP"])
async def detect_dialect(req: TextRequest):
    """Detect dialect and normalize."""
    result, ms = process_with_timing(manglish_nlp.detect_dialect, req.text)
    return NLPResponse(result=result, processing_time_ms=round(ms, 2))
...
async def batch_process(req: BatchRequest):
    ...
    module_map = {
        "sentiment": manglish_nlp.sentiment,
        "normalize": manglish_nlp.normalize,
        "ner": manglish_nlp.ner_tag,
        "pos": manglish_nlp.pos_tag,
        "translate": manglish_nlp.to_english,
        "emotion": manglish_nlp.detect_emotion,
        "keywords": manglish_nlp.extract_keywords,
        "language": manglish_nlp.detect_language,
        "formalize": manglish_nlp.formalize,
        "summarize": manglish_nlp.summarize,
    }
