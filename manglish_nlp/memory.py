"""Memory optimization utilities for manglish_nlp.
...
Usage::
    ...
    from manglish_nlp.memory import ModuleRegistry, optimize_memory
...
class LazyModule:
    ...
    Args:
        module_name: Fully qualified module name (e.g. ``'manglish_nlp.sentiment'``).
        package: Optional package context for relative imports.

    Example::

        sentiment = LazyModule('manglish_nlp.sentiment')
...
# All known manglish_nlp submodules
...
class ModuleRegistry:
    """Registry that lazily loads manglish_nlp modules on access.

    Access any module as an attribute::

        reg = ModuleRegistry()
        reg.sentiment   # imports manglish_nlp.sentiment on first access
        reg.ner         # imports manglish_nlp.ner on first access
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name not in instances:
            ...
            mod = importlib.import_module(f'manglish_nlp.{name}')
    ...
    def is_loaded(self, name: str) -> bool:
        ...
        Args:
            name: Module name (without ``manglish_nlp.`` prefix).
    ...
    def memory_info(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict with:
                ...
                - ``sys_modules_manglish``: count of manglish_nlp modules in sys.modules
        """
        instances = object.__getattribute__(self, '_instances')
        load_times = object.__getattribute__(self, '_load_times')
        access_counts = object.__getattribute__(self, '_access_counts')

        # Count manglish_nlp modules in sys.modules
        manglish_count = sum(
            1 for k in sys.modules if k.startswith('manglish_nlp')
        )
...
def optimize_memory() -> Dict[str, Any]:
    ...
    tracked objects, and manglish_nlp module footprint.
    ...
    manglish_modules = sorted(
        k for k in sys.modules if k.startswith('manglish_nlp')
    )
...
def get_all_module_sizes() -> List[Dict[str, Any]]:
    """Estimate memory usage for all loaded manglish_nlp modules.
    ...
    for name in sorted(sys.modules):
        if name.startswith('manglish_nlp'):
