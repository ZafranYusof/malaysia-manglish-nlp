...
def profile_all_modules(text, iterations=100):
...
def profile_module(module_name, text, iterations=1000):
...
def memory_usage():
    ...
    for name, mod in sys.modules.items():
        if not name.startswith('manglish_nlp'):
            continue
        if mod is None:
            continue
        
        short_name = name.replace('manglish_nlp.', '').replace('manglish_nlp', '__init__')
...
def benchmark_throughput(texts, module='sentiment'):
...
def generate_report(output_path=None):
    ...
    lines.append("")
    try:
        from manglish_nlp.cache import cache_stats
        stats = cache_stats()
        if stats:
...
def _call_module(module_name, text):
    ...
    if module_name == 'normalize':
        ...
        from manglish_nlp.stemmer import stem
        return stem(text)
    elif module_name == 'tokenizer':
        from manglish_nlp.tokenizer import tokenize
        ...
        from manglish_nlp.segment import segment
        return segment(text)
        ...
        from manglish_nlp.dictionary import is_malay
