...
def _find_indicators(text, indicator_list):
...
def _check_negation(text, indicator):
...
def _is_relevant_to_target(text, target):
...
def detect_stance(text, target=None):
...
def detect_stance_batch(texts, target=None):
...
def compare_stances(text1, text2):
...
def extract_stance_target(text):
