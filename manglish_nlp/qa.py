...
def _word_overlap_score(text1, text2):
...
def _tfidf_score(query, sentences):
...
def extract_answer_span(sentence, question):
...
def _extract_where(sentence, question):
...
def _extract_who(sentence, question):
...
def _extract_what(sentence, question):
...
def _extract_yes_no(sentence, question):
...
def _calculate_confidence(context, question, sentence, answer, qtype):
...
def _find_span_position(context, answer):
...
def answer_multiple(context, questions):
