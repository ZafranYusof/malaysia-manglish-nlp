...
Usage:
    python -m manglish_nlp.evaluate          # Run full eval
    python -m manglish_nlp.evaluate --save   # Save results to history
    python -m manglish_nlp.evaluate --compare  # Compare with last saved
"""
...
def run_evaluation():
    ...
    # Sentiment
    passed = sum(1 for t, e in SENTIMENT_DATA if manglish_nlp.sentiment(t)['sentiment'] == e)
    results['sentiment'] = {'passed': passed, 'total': len(SENTIMENT_DATA), 'accuracy': round(passed / len(SENTIMENT_DATA) * 100, 1)}
    
    # Language
    passed = sum(1 for t, e in LANGUAGE_DATA if manglish_nlp.detect_language(t)['language'] == e)
    ...
    # Normalization
    passed = sum(1 for t, e in NORM_DATA if manglish_nlp.normalize(t) == e)
    results['normalization'] = {'passed': passed, 'total': len(NORM_DATA), 'accuracy': round(passed / len(NORM_DATA) * 100, 1)}
    
    # Stemmer
    passed = sum(1 for t, e in STEMMER_DATA if manglish_nlp.stem_word(t) == e)
