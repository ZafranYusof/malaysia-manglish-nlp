...
def print_json(data, colored=True):
...
def get_text_from_args_or_stdin(args):
...
def cmd_analyze(args):
    ...
    # Normalize
    result, ms = timed(manglish_nlp.normalize, text)
    print(f"  {c(Colors.BOLD, 'Normalized:')}  {result} {c(Colors.DIM, f'({ms:.1f}ms)')}")

    # Language detection
    result, ms = timed(manglish_nlp.detect_language, text)
    ...
    # Sentiment
    result, ms = timed(manglish_nlp.sentiment, text)
    ...
    # Emotion
    result, ms = timed(manglish_nlp.detect_emotion, text)
    emo = result.get('emotion', result.get('primary', ''))
    print(f"  {c(Colors.BOLD, 'Emotion:')}     {emo} {c(Colors.DIM, f'({ms:.1f}ms)')}")

    # Keywords
    result, ms = timed(manglish_nlp.extract_keywords, text)
    if result:
...
def cmd_translate(args):
    ...
    if target == 'en':
        result, ms = timed(manglish_nlp.to_english, text)
    elif target in ('bm', 'ms', 'malay'):
        result, ms = timed(manglish_nlp.to_malay, text)
    elif target == 'formal':
        result, ms = timed(manglish_nlp.to_formal, text)
    else:
        result, ms = timed(manglish_nlp.translate, text)
...
def cmd_summarize(args):
    ...
    result, ms = timed(manglish_nlp.summarize, text)
...
def cmd_benchmark(args):
    ...
    modules = [
        ...
        ("pos_tag", manglish_nlp.pos_tag),
        ("ner_tag", manglish_nlp.ner_tag),
        ("detect_language", manglish_nlp.detect_language),
        ("detect_emotion", manglish_nlp.detect_emotion),
        ("extract_keywords", manglish_nlp.extract_keywords),
        ("tokenize", manglish_nlp.tokenize),
        ("stem", manglish_nlp.stem),
        ("clean", manglish_nlp.clean),
    ]
