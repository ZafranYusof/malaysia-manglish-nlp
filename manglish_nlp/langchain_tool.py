...
Usage:
    from manglish_nlp.langchain_tool import ManglishNLPTool, ManglishSentimentTool
...
class ManglishNLPTool(BaseTool):
    ...
    def _run(
        ...
        return {
            ...
            "language": manglish_nlp.detect_language(text),
            "pos_tags": manglish_nlp.pos_tag(text),
            "entities": manglish_nlp.ner_tag(text),
            "emotion": manglish_nlp.detect_emotion(text),
            "keywords": manglish_nlp.extract_keywords(text),
        }
...
class ManglishTranslateTool(BaseTool):
    ...
    def _run(
        ...
        target = target.lower()
        if target == "en":
            return manglish_nlp.to_english(text)
        elif target in ("bm", "ms", "malay"):
            return manglish_nlp.to_malay(text)
        elif target == "formal":
            return manglish_nlp.to_formal(text)
        else:
            return manglish_nlp.translate(text)
