...
other malaysian_manglish_nlp modules across multiple CPU cores using either
...
def _worker_tokenize(text: str) -> List[str]:
    ...
    from malaysian_manglish_nlp.tokenizer_fast import tokenize
    return tokenize(text)


def _worker_normalize(text: str) -> str:
    """Normalize a single text (worker)."""
    from malaysian_manglish_nlp.tokenizer_fast import normalize
...
def _worker_generic(text: str, module_name: str, func_name: str) -> Any:
    ...
    import importlib
    mod = importlib.import_module(f'malaysian_manglish_nlp.{module_name}')
...
class ParallelPipeline:
    ...
    def __init__(
        ...
        Args:
            n_workers: N workers parameter.
            use_threads: Use threads parameter.
