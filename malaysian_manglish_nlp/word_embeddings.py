...
def generate_corpus(n_sentences: int = 2000, seed: Any = 42) -> List[str]:
...
def _generate_sentence(templates: Any) -> List[str]:
...
def train_fasttext(corpus: Optional[Any] = None, vector_size: int = 100, window: int = 5, min_count: int = 2) -> Any:
...
def train_all(vector_size: int = 50, window: int = 5, min_count: int = 2, n_sentences: int = 2000) -> Dict[str, Any]:
...
def _get_model(model_type: str = 'fasttext') -> Any:
...
def sentence_vector(text: str, model_type: str = 'fasttext') -> List[float]:
